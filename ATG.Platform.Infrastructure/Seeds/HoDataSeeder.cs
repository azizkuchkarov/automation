...
public static class HoDataSeeder
{
    ...
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        var ho = await db.Organizations.FirstOrDefaultAsync(o => o.Code == HoMasterData.OrganizationCode);
        if (ho is null) return;

        logger.LogInformation("Syncing Tashkent Head Office structure...");

        var deptIds = await DepartmentSeederHelper.SyncDepartmentsAsync(
            db, ho.Id, HoMasterData.Departments, HoMasterData.LegacyDepartmentCodes);

        var positions = await db.Positions.ToDictionaryAsync(p => p.Code, p => p.Id, StringComparer.OrdinalIgnoreCase);
        var staffCreated = 0;
        var staffUpdated = 0;

        foreach (var member in HoMasterData.Staff)
        {
        ...
        if (admin is not null && deptIds.TryGetValue("HO-ITDIG", out var itDeptId))
            admin.DepartmentId = itDeptId;

        await db.SaveChangesAsync();
        logger.LogInformation("HO sync complete: {Created} users created, {Updated} updated", staffCreated, staffUpdated);
    }
